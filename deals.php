...
<div class="full_article_bg">
	...
	if(isset($_GET['category']) == true || isset($_POST['query']) == true){
		...
		if($count==0){
			echo "<div id='contentSub' class='clearfix'>
					  <div class='contentBox'>
						  ...
						  <p class='noKorksCreate'><a href='/manage_deals/new' class='entypo-pencil'> Create Your Deals!</a></p>
					  </div>
				  </div>";
		}
		...
		{		
				...
				echo "<div id='action' style='' class='prod_desc'><a href='/$_username/{$title_withDashes}/{$id}'>";
				// echo "<span class='$status korkbadge'></span>";
				echo "<img class='main-prod-pic' src='/img/korkImages/$image' width='247px' height='237px' alt=''>
                <a href='/manage_deals/{$title_withDashes}/{$id}' target='_blank'><input title='Edit' class='update_button' type='button' value='' /></a>"
                    ;
				echo "<div class='details'>";
				echo "<h3 class='block-ellipsis' style='font-weight:bold;'>$title</h3></a>";
				echo "<h3 class='details-block-ellipsis' style='font-size: 14px;margin-top: 14px;'> $detail </h3></a>";
				echo "<p>By: <a href='/$username'>$username</a></p><p class='detail_timestamp'><span>".date('m-d-Y | h:i A', strtotime($expiryDate))."</p>
					 <div class='price' style='margin-top: 38px !important;'><span class='price_first' style='width: 36%'>Rs. {$price}</span><span class='prod_scheme'>&nbsp; {$bids} <span class='off'>BIDS</span></span></div>
